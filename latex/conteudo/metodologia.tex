...
A estrutura da aplicação pode ser compreendida a partir de três partes principais. A primeira corresponde à apresentação, implementada em ambiente web, na qual o usuário constrói visualmente a topologia da rede e acompanha os resultados dos cálculos em tempo real. A segunda refere-se ao processamento, responsável por tratar as operações solicitadas, validar os parâmetros informados e executar as rotinas de cálculo do orçamento óptico com base nos elementos modelados. A terceira diz respeito aos dados, destinados ao armazenamento estruturado das informações do projeto, incluindo topologia, parâmetros técnicos e resultados calculados. Essa arquitetura será complementada por diagramas e fluxos de interação, de modo a explicitar a comunicação entre as partes do sistema e facilitar a compreensão de seu funcionamento.
...
A validação da solução será realizada por meio de cenários de teste representativos do planejamento de redes ópticas \gls{pon}, nos quais serão modeladas topologias com diferentes comprimentos de fibra, quantidades de emendas, conectores e configurações de \textit{splitters}. Os instrumentos de coleta serão constituídos pelos próprios artefatos gerados e processados pelo sistema, incluindo topologias modeladas, parâmetros de entrada, valores calculados, registros de execução e respostas fornecidas pela interface e pelo \textit{backend}. Para cada cenário, os valores produzidos serão comparados com referências previamente calculadas ou obtidas a partir de parâmetros técnicos consolidados na literatura, a fim de verificar a correção das rotinas implementadas, conforme práticas empregadas na validação de sistemas computacionais \cite{pressman2014software}. Além da comparação numérica, serão observados o comportamento da interface durante a construção e atualização da topologia e a capacidade de armazenamento e recuperação dos projetos em formato \gls{json}. O fluxo geral desse procedimento é sintetizado na Figura~\ref{fig:fluxo_validacao}, que apresenta as etapas de definição dos cenários, execução da aplicação, coleta dos registros gerados e exame dos resultados obtidos.
...
A avaliação da solução considerará critérios como a correção das operações de modelagem da topologia, a consistência dos valores de orçamento óptico, o comportamento do software frente a diferentes estruturas de rede e a integridade dos dados. Adicionalmente, serão observadas métricas complementares, como tempo de resposta, estabilidade durante o uso e clareza das informações apresentadas, alinhadas a critérios de avaliação de desempenho em sistemas de software \cite{sommerville2016software}. Do ponto de vista ético, o trabalho limita-se à análise técnica de um sistema computacional, sem envolvimento de dados pessoais ou participantes humanos, adotando princípios de integridade científica, rastreabilidade dos testes e transparência na apresentação dos resultados. A ferramenta será considerada satisfatória caso atenda aos critérios definidos e demonstre aderência aos objetivos propostos.
