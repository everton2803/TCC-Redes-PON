...
\begin{itemize}
    \item \textbf{Criação e Edição Interativa da Topologia:} O sistema deve permitir ao usuário construir e modificar visualmente a árvore da rede, inserindo, movendo e removendo elementos como \gls{olt}, caixas de emenda/fusão, \textit{splitters} (balanceados e desbalanceados) e \gls{onu}. Elementos sem comportamento próprio no cálculo, como CTOs, podem ser identificados por anotações visuais.
    ...
    \item \textbf{Parametrização dos Equipamentos:} O sistema deve permitir informar a potência de transmissão ($T_x$) da \gls{olt} e a sensibilidade de recepção ($R_x$) das \glspl{onu}, possibilitando representar diferentes equipamentos por meio de parâmetros customizados.
    
    \item \textbf{Indicação do Status do Link:} O sistema deve sinalizar, após alterações na topologia ou nos parâmetros, se a potência óptica que atinge cada nó de terminação \gls{onu} está dentro dos limites operacionais adequados, exibindo alertas em caso de sinal degradado ou fora da margem de sensibilidade.
...
\begin{itemize}
    ...
    \item \textbf{Renderização Gráfica Interativa:} A interface gráfica (\textit{frontend}) deve utilizar a biblioteca D3.js no navegador para permitir a manipulação dos nós e enlaces, o zoom, o arraste e a atualização dos indicadores. O comportamento em redes densas deve ser avaliado por testes de desempenho, não presumido como garantia para qualquer escala.
    ...
    \item \textbf{Tempo de Resposta Avaliável:} O tempo de resposta das chamadas à \gls{api} REST para recálculo do orçamento óptico deve ser medido em cenários controlados, considerando o tamanho da topologia e a quantidade de caminhos analisados.
...
\subsection{Prototipação da Interface (\textit{Wireframes})}

Antes do início da codificação da interface gráfica final, elaborei protótipos de interface em \textit{wireframes}. O objetivo dessa etapa foi organizar a arquitetura de informação e validar a usabilidade do operador antes da integração com a biblioteca gráfica D3.js e o motor de cálculo.
...
\begin{itemize}
    ...
    \item \textbf{Painel de Propriedades e Orçamento Óptico:} Coluna lateral à direita dedicada à edição dos parâmetros do nó selecionado e exibição do nível de sinal calculado após o recálculo da topologia.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Requisição HTTP:} A aplicação envia uma requisição \gls{http} \texttt{POST} assíncrona contendo o estado da rede para a rota \texttt{/calculo/orcamento}.
...
router = APIRouter(prefix="/calculo", tags=["Cálculo Óptico"])
...
async function calcularOrcamentoOptico() {
    try {
        const response = await fetch('/calculo/orcamento', {
            method: 'POST',
